...
### 1a. `page_content` — the actual text

- It's just a **plain Python string** (`str`) — nothing fancy.
- It holds whatever text the `Document` represents: a whole file, a single PDF
  page, a CSV row, a chunk after splitting, a scraped webpage's text, etc.
- This is the field that actually gets **embedded** (turned into a vector) and
  **shown to the LLM** as context during retrieval — it's the "payload."
- It's mutable — you can edit it directly if needed:
  ```python
  doc.page_content = doc.page_content.strip().lower()
  ```
- There is no fixed size limit on `page_content` from the `Document` class
  itself, but in practice you keep it small using a **text splitter** (see
  section 7) because embedding models and LLM context windows have limits.

### 1b. `metadata` — data *about* the text

- It's a plain Python **dictionary** (`dict`) — any keys/values you want.
- It doesn't get embedded — it's not part of what determines "similarity" in a
  vector search by default. It travels alongside `page_content` as extra
  context.
- Typical uses:
  - **Traceability** — `source` (file path or URL), so you can tell the user
    *where* an answer came from.
  - **Filtering** — e.g. only search documents where `metadata["author"] ==
    "khaja"` or `metadata["date"] > "2026-01-01"`. Most vector stores support
    filtering search results by metadata.
  - **Citation / display** — page numbers, titles, row numbers, timestamps.
  - **Debugging** — knowing exactly which file/row/page a chunk came from
    when something looks wrong.
- **Loaders auto-fill metadata for you.** You don't have to add it by hand
  when using a loader — each loader adds fields relevant to its source type:

  | Loader | Typical metadata added automatically |
  |---|---|
  | `TextLoader` | `source` (file path) |
  | `PyPDFLoader` | `source` (file path), `page` (page number, 0-indexed) |
  | `CSVLoader` | `source` (file path), `row` (row number) |
  | `DirectoryLoader` | whatever the inner `loader_cls` adds, per file |

  Example:
  ```python
  documents[0].metadata
  # {'source': './data/intro.txt'}

  docs[0].metadata
  # {'source': './data/sample.pdf', 'page': 0}
  ```

- **Metadata carries through splitting.** When a `Document` is split into
  chunks by a text splitter, every chunk **inherits a copy of the parent's
  metadata** (so each chunk still knows which file/page it came from), and
  splitters often *add to it* — e.g. `MarkdownHeaderTextSplitter` adds the
  heading trail (`Header 1`, `Header 2`, ...) so each chunk remembers which
  section of the document it belongs to.

- **You can add your own metadata** after loading, which is common when you
  want extra fields the loader doesn't provide (e.g. a category tag, an
  access-control label, a language code):
  ```python
...
**Rule of thumb:** `page_content` = "what the model reads and searches over."
`metadata` = "what your application uses to filter, cite, and trace results
back to their source" — it's the difference between *content* and *context*.
...
loader = SomeLoader(file_path="...")   # TextLoader / PyPDFLoader / CSVLoader
docs = loader.load()                   # always returns list[Document]

# DirectoryLoader just automates calling this for many files:
dir_loader = DirectoryLoader(path="./data", glob="*.txt", loader_cls=TextLoader)
